...
class SoftTreeArgs():
    def __init__(self,input_dim,output_dim,
                 batch_size=16,device='cpu',lmbda=1,max_depth=3,lr=0.001,momentum=0.1,log_interval=10):
...
rand_y = torch.long(rand_y)
envs = [DataLoader(TensorDataset(rand_X[i,:,:],rand_y[i,:]),batch_size=10) for i in range(NUM_ENVS)]
#rand_trees = [DecisionTreeClassifier(max_depth=3).fit(X,y) for X,y in zip(rand_X,rand_y)]
args = SoftTreeArgs(input_dim=4,output_dim=1)
